...
export async function POST(request: Request) {
  ...
  if (!token) return NextResponse.json({ error: 'Conectá tu propia cuenta de Mercado Pago antes de crear un checkout.' }, { status: 401 })
  ...
  try {
    ...
    if (!response.ok) return NextResponse.json({ error: 'Mercado Pago rechazó la preferencia.', details: data }, { status: 502 })
