...
export async function GET(req:Request){
 ...
 if(!code||!state||!expected||state!==expected){back.searchParams.set('error','La autorización de Mercado Pago no pudo verificarse. Volvé a intentarlo.');return NextResponse.redirect(back)}
 try{
  const {clientId,clientSecret,redirectUri}=oauthConfig()
  const response=await fetch('https://api.mercadopago.com/oauth/token',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({client_id:clientId,client_secret:clientSecret,code,grant_type:'authorization_code',redirect_uri:redirectUri})})
  ...
  const expiresAt=Date.now()+Number(data.expires_in||15552000)*1000
  saveOAuthToken({access_token:data.access_token,refresh_token:data.refresh_token,expires_at:expiresAt,user_id:String(data.user_id||'')})
