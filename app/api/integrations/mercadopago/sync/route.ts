...
async function refreshIfNeeded(token:{access_token:string;refresh_token?:string;expires_at?:number}){
...
export async function GET(){
 try{
  ...
  const movements=rows.map((p:any)=>{
   ...
   return {id:String(p.id),date:p.date_approved||p.date_created||new Date().toISOString(),type,title:approved?'Cobro Mercado Pago':'Movimiento Mercado Pago',amount,client:payer.first_name||payer.last_name?[payer.first_name,payer.last_name].filter(Boolean).join(' '):payer.email||undefined,dni:identification.type==='DNI'?identification.number:undefined,status:p.status,detail:p.description||p.external_reference||''}
  }).filter((m:any)=>m.amount>0)
  return NextResponse.json({connected:true,movements,user:{id:me?.id,name:[me?.first_name,me?.last_name].filter(Boolean).join(' '),email:me?.email},message:`Mercado Pago sincronizado: ${movements.length} movimientos.`})
 }catch(err){
  const status=err instanceof Error&&err.message.includes('Primero conectá')?401:500
  return NextResponse.json({connected:false,error:err instanceof Error?err.message:'No se pudo sincronizar Mercado Pago.'},{status})
 }
}
