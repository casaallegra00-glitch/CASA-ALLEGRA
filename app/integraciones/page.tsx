const integrations = [
  {
    ...
    name: 'Mercado Pago',
    type: 'Pagos',
    text: 'Preparado para pagos online, links de pago, referencias y estados de cobro.',
  },
  {
    icon: '🛒',
    name: 'Mercado Libre',
    type: 'Marketplace',
    text: 'Preparado para publicaciones, SKU, ventas, compradores y pedidos.',
  },
  {
    icon: '📦',
    name: 'Mercado Envíos',
    type: 'Envíos',
    text: 'Preparado para shipment, tracking, estados y costos de envío.',
  },
  {
    icon: '📮',
    name: 'Correo Argentino',
    type: 'Correo',
    text: 'Preparado para despacho, seguimiento, estado, costo y entrega.',
  },
  {
    icon: '🚚',
    name: 'Andreani',
    type: 'Logística',
    text: 'Preparado para despacho, tracking, estado de entrega y costo.',
  },
]

export default function IntegracionesPage() {
  return (
    <main style={{ minHeight: '100vh', padding: '32px', background: '#fffaf7', color: '#3c3441' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(210px,1fr))', gap: 16 }}>
          {integrations.map((item) => (
            <article key={item.name} style={{ background: '#fff', border: '1px solid #eee4ef', borderRadius: 20, padding: 20, boxShadow: '0 15px 45px rgba(67,43,88,.09)', minHeight: 230, display: 'flex', flexDirection: 'column' }}>
              ...
              <button type="button" onClick={() => window.alert(`${item.name}: interfaz preparada para conectar la API oficial.`)} style={{ marginTop: 'auto', border: 0, borderRadius: 13, padding: '11px 14px', background: '#63c7c9', color: '#fff', fontWeight: 800, cursor: 'pointer' }}>
                Configurar
              </button>
            </article>
          ))}
          ...
          <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            {['🧾 Presupuesto', '💳 Pago', '📦 Pedido', '🚚 Envío', '✅ Entrega'].map((step, index) => (
              <span key={step} style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                <span style={{ padding: '9px 11px', background: '#f8f3fa', borderRadius: 10, fontSize: 11, fontWeight: 800 }}>{step}</span>
                {index < 4 && <b>→</b>}
              </span>
            ))}
          </div>
