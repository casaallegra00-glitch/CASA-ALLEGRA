...
export default function IntegracionesPage() {
    ...
    fetch('/api/mercadopago/health', { cache: 'no-store' })
      .then(async (response) => {
        ...
        if (data.connected) {
          ...
          setMpMessage(`Conectado${data.nickname ? ` · ${data.nickname}` : ''}`)
        } else if (!data.configured) {
          setMpStatus('not-configured')
          setMpMessage('Falta configurar MERCADOPAGO_ACCESS_TOKEN en Vercel.')
        } else {
          setMpStatus('invalid')
          setMpMessage(data.message || 'No se pudo validar el Access Token.')
        }
      })
      .catch(() => {
        setMpStatus('invalid')
        setMpMessage('No se pudo consultar el estado de Mercado Pago.')
      })
  }, [])
  ...
  const createTestCheckout = async () => {
    ...
    if (!Number.isFinite(numericAmount) || numericAmount <= 0) {
      setCheckoutError('Ingresá un importe válido mayor que 0.')
      return
    }
    const response = await fetch('/api/mercadopago/preferences', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title: 'Prueba CASA ALLEGRA', quantity: 1, unit_price: numericAmount }),
    })
    const data = await response.json().catch(() => ({}))
    if (!response.ok) {
      setCheckoutError(data.error || 'No se pudo crear el checkout.')
      return
    }
    ...
    <main style={{ minHeight: '100vh', padding: 32, background: '#fffaf7', color: '#3c3441' }}>
      <div style={{ maxWidth: 1180, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <p style={{ margin: 0, color: '#7d7381' }}>Centro de pagos, marketplace y logística. Las claves privadas se utilizan únicamente en el servidor de Vercel.</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(210px,1fr))', gap: 16 }}>
          {integrations.map((item) => (
            <article key={item.name} style={{ background: '#fff', border: '1px solid #eee4ef', borderRadius: 20, padding: 20, boxShadow: '0 15px 45px rgba(67,43,88,.09)', minHeight: 245, display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ marginTop: 10, fontSize: 11, fontWeight: 800, color: item.name === 'Mercado Pago' ? (mpStatus === 'connected' ? '#4f946f' : '#9a8f9d') : (connected[item.name] ? '#4f946f' : '#9a8f9d') }}>
                {item.name === 'Mercado Pago' ? (mpStatus === 'connected' ? '● Conectado' : mpStatus === 'checking' ? '○ Comprobando…' : '○ Sin conectar') : (connected[item.name] ? '● Preparada' : '○ Sin configurar')}
              </div>
        ...
        <section style={{ marginTop: 18, background: '#fff', border: '1px solid #eee4ef', borderRadius: 20, padding: 20 }}>
          ...
          <p style={{ margin: '8px 0 14px', color: '#7d7381', fontSize: 13 }}>{mpMessage}</p>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
            <label style={{ fontSize: 12, fontWeight: 800 }}>Importe ARS<input value={amount} onChange={(e) => setAmount(e.target.value)} inputMode="decimal" style={{ display: 'block', marginTop: 5, width: 150, border: '1px solid #e9dfea', borderRadius: 12, padding: '10px 12px' }} /></label>
            <button type="button" onClick={createTestCheckout} style={{ marginTop: 17, border: 0, borderRadius: 13, padding: '11px 14px', background: '#63c7c9', color: '#fff', fontWeight: 800, cursor: 'pointer' }}>Crear checkout de prueba</button>
            {checkoutUrl && <a href={checkoutUrl} target="_blank" rel="noreferrer" style={{ marginTop: 17, borderRadius: 13, padding: '11px 14px', background: '#f8f3fa', color: '#3c3441', fontWeight: 800, textDecoration: 'none' }}>Abrir Mercado Pago</a>}
          </div>
        ...
        {selected && (
          <div role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, background: 'rgba(49,38,61,.42)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 18, zIndex: 10000 }}>
            <div style={{ width: 'min(560px,100%)', background: '#fff', borderRadius: 20, padding: 22 }}>
              ...
              <p style={{ color: '#7d7381', fontSize: 13 }}>{selected.name === 'Mercado Pago' ? 'La conexión real utiliza MERCADOPAGO_ACCESS_TOKEN en Vercel. No pegues aquí un token productivo.' : 'Estos campos son la estructura de configuración; la conexión real se incorporará por servidor.'}</p>
              {selected.name !== 'Mercado Pago' && <div style={{ display: 'grid', gap: 10, marginTop: 16 }}>
                {selected.fields.map((field) => (
                  <label key={field} style={{ display: 'grid', gap: 6, fontSize: 11, fontWeight: 800 }}>{field}<input value={values[field] || ''} onChange={(e) => setValues((current) => ({ ...current, [field]: e.target.value }))} placeholder="Preparado para conexión segura" style={{ border: '1px solid #e9dfea', borderRadius: 12, padding: '10px 12px', outline: 'none' }} /></label>
                ))}
              </div>}
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 18 }}>
                ...
                {selected.name !== 'Mercado Pago' && <button type="button" onClick={saveConnection} style={{ border: 0, borderRadius: 13, padding: '11px 14px', background: '#63c7c9', color: '#fff', fontWeight: 800, cursor: 'pointer' }}>Guardar estructura</button>}
              </div>
            </div>
          </div>
        )}
