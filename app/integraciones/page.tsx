...
export default function IntegracionesPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [mp, ml] = await Promise.all([
        fetch('/api/integraciones/mercadopago/status', { cache: 'no-store' }).then(r => r.json()).catch(() => ({ connected: false })),
        fetch('/api/integraciones/mercadolibre/status', { cache: 'no-store' }).then(r => r.json()).catch(() => ({ connected: false })),
      ])
      if (!active) return
  ...
  const createCheckout = async () => {
    ...
    const response = await fetch('/api/mercadopago/preferences', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: 'Prueba CASA ALLEGRA', quantity: 1, unit_price: value }) })
    const body = await response.json().catch(() => ({}))
    ...
    <main style={{ minHeight: '100vh', padding: 32, background: '#fffaf7', color: '#3c3441' }}>
      <div style={{ maxWidth: 1180, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 12, letterSpacing: '.12em', fontWeight: 800, color: '#36aeb2' }}>CASA ALLEGRA APP</div>
          <h1 style={{ margin: '6px 0 8px', fontSize: 36 }}>🔗 Conexiones de tu negocio</h1>
          <p style={{ margin: 0, color: '#7d7381', lineHeight: 1.6 }}>Cada usuario conecta sus propias cuentas. CASA ALLEGRA no comparte tus cuentas con otros usuarios.</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(215px,1fr))', gap: 16 }}>
          {cards.map(card => {
            ...
            return <article key={card.name} style={{ background: '#fff', border: '1px solid #eee4ef', borderRadius: 20, padding: 20, minHeight: 260, boxShadow: '0 15px 45px rgba(67,43,88,.09)', display: 'flex', flexDirection: 'column' }}>
              <div style={{ fontSize: 34 }}>{card.icon}</div>
              <span style={{ marginTop: 10, alignSelf: 'flex-start', padding: '5px 8px', borderRadius: 999, background: '#eaf8f7', color: '#337d80', fontSize: 10, fontWeight: 800 }}>{card.kind}</span>
              <h2 style={{ margin: '10px 0 8px', fontSize: 20 }}>{card.name}</h2>
              <p style={{ margin: 0, color: '#7d7381', lineHeight: 1.5, fontSize: 13 }}>{card.description}</p>
              <div style={{ marginTop: 12, fontSize: 11, fontWeight: 800, color: providerStatus.startsWith('●') ? '#4f946f' : '#9a8f9d' }}>{providerStatus}</div>
              {card.provider ? <button type="button" onClick={() => connect(card.provider!)} style={{ marginTop: 'auto', border: 0, borderRadius: 13, padding: '12px 14px', background: '#63c7c9', color: '#fff', fontWeight: 800, cursor: 'pointer' }}>{status[card.provider] === 'connected' ? 'Administrar conexión' : 'Conectar con 1 clic'}</button> : <button type="button" onClick={() => setMessage(card.note || 'Esta conexión se implementará con las credenciales del proveedor.')} style={{ marginTop: 'auto', border: '1px solid #eee4ef', borderRadius: 13, padding: '12px 14px', background: '#fff', color: '#3c3441', fontWeight: 800, cursor: 'pointer' }}>Ver cómo conectar</button>}
            </article>
          })}
        </div>

        <section style={{ marginTop: 18, background: '#fff', border: '1px solid #eee4ef', borderRadius: 20, padding: 20 }}>
          <h2 style={{ margin: 0 }}>💳 Checkout Pro con la cuenta del usuario</h2>
          <p style={{ color: '#7d7381', lineHeight: 1.55 }}>Una vez conectado Mercado Pago, los cobros de este usuario se crean usando su propia autorización OAuth, no el Access Token global de CASA ALLEGRA.</p>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'end' }}>
            <label style={{ fontSize: 12, fontWeight: 800 }}>Importe ARS<input value={amount} onChange={e => setAmount(e.target.value)} style={{ display: 'block', marginTop: 5, width: 150, border: '1px solid #e9dfea', borderRadius: 12, padding: '10px 12px' }} /></label>
            <button type="button" onClick={createCheckout} disabled={status.mercadopago !== 'connected'} style={{ border: 0, borderRadius: 13, padding: '11px 14px', background: status.mercadopago === 'connected' ? '#63c7c9' : '#cfc7d1', color: '#fff', fontWeight: 800 }}>Crear checkout de prueba</button>
            {checkoutUrl && <a href={checkoutUrl} target="_blank" rel="noreferrer" style={{ borderRadius: 13, padding: '11px 14px', background: '#f8f3fa', color: '#3c3441', fontWeight: 800, textDecoration: 'none' }}>Abrir Mercado Pago</a>}
          </div>
        ...
        <section style={{ marginTop: 18, background: '#fff', border: '1px solid #eee4ef', borderRadius: 20, padding: 18 }}>
          ...
          <div style={{ marginTop: 12, display: 'grid', gap: 8, color: '#6f6673', fontSize: 13 }}>
            <div>1. El usuario inicia sesión en CASA ALLEGRA.</div>
            <div>2. Pulsa <b>Conectar con 1 clic</b>.</div>
            <div>3. Mercado Pago o Mercado Libre muestra su propia pantalla de autorización.</div>
            <div>4. El usuario acepta y vuelve automáticamente a CASA ALLEGRA.</div>
            <div>5. El token queda protegido en una cookie HttpOnly cifrada y asociado al usuario que autorizó.</div>
          </div>
        </section>
      </div>
    </main>
