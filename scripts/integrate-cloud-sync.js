...
"const syncNow=async()=>{if(cancelled||syncing)return;syncing=true;try{migrate();const local=snapshot();const auth=await db.auth.getUser();const uid=auth.data.user?.id;if(!uid){setStorageReady(true);return}const result=await db.from('business_state').select('payload,updated_at').eq('user_id',uid).maybeSingle();if(result.error)throw result.error;const cloud=result.data?.payload?.storage&&typeof result.data.payload.storage==='object'?result.data.payload.storage as Record<string,string>:{};const merged=merge(local,cloud);apply(merged);const payload={version:5,storage:snapshot()};const saved=await db.from('business_state').upsert({user_id:uid,payload,updated_at:new Date().toISOString()},{onConflict:'user_id'});if(saved.error)throw saved.error;sessionStorage.setItem('casa-allegra-cloud-last-snapshot',JSON.stringify(snapshot()))}catch(err){console.warn('CASA ALLEGRA: recuperación cloud no disponible.',err);setStorageReady(true)}finally{syncing=false}};",
