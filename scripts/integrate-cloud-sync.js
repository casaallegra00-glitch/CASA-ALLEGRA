...
"const snapshot=()=>{const out={};for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(k&&k.startsWith(prefix)&&k!=='casa-allegra-cloud-last-snapshot'&&k!=='casa-allegra-zero-data-v1')out[k]=localStorage.getItem(k)||''}return out};",
...
"const merge=(local,cloud)=>{const out={...cloud,...local};for(const k of new Set([...Object.keys(cloud),...Object.keys(local)])){const l=local[k],c=cloud[k];if(empty(l)&&!empty(c)){out[k]=c;continue}if(empty(c)&&!empty(l)){out[k]=l;continue}if(l==null){out[k]=c||'';continue}if(c==null){out[k]=l;continue}try{const a=JSON.parse(l),b=JSON.parse(c);if(Array.isArray(a)&&Array.isArray(b)){const all=[...b,...a],seen=new Set(),m=[];for(const item of all){const id=item&&typeof item==='object'?(item.id??item.number??item.sku):null;const sig=id!=null?String(id):JSON.stringify(item);if(!seen.has(sig)){seen.add(sig);m.push(item)}}out[k]=JSON.stringify(m)}else out[k]=l||c}catch{out[k]=l||c}}return out};",
"const write=d=>{for(const [k,v] of Object.entries(d)){try{localStorage.setItem(k,v)}catch{}}};",
"const apply=d=>{write(d);setProducts(load(base+'-products',[]));setSales(load(base+'-sales',[]));setClients(load(base+'-clients',[]));setOrders(load(base+'-orders',[]));setCash(load(base+'-cash',[]));setBusinessCategories(load(base+'-categories',['General','Productos','Servicios','Otros']));const n=load(base+'-business','');if(n)setBusinessName(n);setStorageReady(true)};",
"const syncNow=async()=>{if(cancelled||syncing)return;syncing=true;try{migrate();const local=snapshot();const auth=await db.auth.getUser();const uid=auth.data.user?.id;if(!uid){setStorageReady(true);return}const result=await db.from('business_state').select('payload,updated_at').eq('user_id',uid).maybeSingle();if(result.error)throw result.error;const cloud=result.data?.payload?.storage&&typeof result.data.payload.storage==='object'?result.data.payload.storage:{};const merged=merge(local,cloud);apply(merged);const payload={version:5,storage:snapshot()};const saved=await db.from('business_state').upsert({user_id:uid,payload,updated_at:new Date().toISOString()},{onConflict:'user_id'});if(saved.error)throw saved.error;sessionStorage.setItem('casa-allegra-cloud-last-snapshot',JSON.stringify(snapshot()))}catch(err){console.warn('CASA ALLEGRA: recuperación cloud no disponible.',err);setStorageReady(true)}finally{syncing=false}};",
