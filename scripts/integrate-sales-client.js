...
const salesBlock = `{section==='ventas'&&<section className="panel large-section"><div className="panel-heading"><div><h2>Ventas</h2><small>Elegí el cliente antes de registrar la venta. El cliente queda asociado al historial.</small></div></div><div className="panel form-panel" style={{marginBottom:18}}><label>Cliente *</label><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><select value={saleClientId??''} onChange={e=>setSaleClientId(e.target.value?Number(e.target.value):null)} style={{flex:1,minWidth:220}}><option value="">Seleccionar cliente...</option>{clients.map(c=><option key={c.id} value={c.id}>{c.name}{c.contact?\` · \${c.contact}\`:''}</option>)}</select><button type="button" className="secondary-btn" onClick={()=>setNewSaleClient(v=>!v)}>＋ Nuevo cliente</button></div>{newSaleClient&&<div className="form-panel" style={{marginTop:12}}><label>Nombre y apellido *</label><input id="saleClientName" placeholder="Ej.: María González"/><label>WhatsApp / teléfono</label><input id="saleClientContact" placeholder="Ej.: 11 1234-5678"/><button type="button" className="primary-btn" onClick={()=>{const name=(document.getElementById('saleClientName') as HTMLInputElement)?.value.trim()||'';const contact=(document.getElementById('saleClientContact') as HTMLInputElement)?.value.trim()||'';if(!name){setNotice('Ingresá el nombre del nuevo cliente.');return}const id=Date.now();setClients(c=>[{id,name,contact},...c]);setSaleClientId(id);setNewSaleClient(false);setNotice(\`Cliente \${name} creado y seleccionado para la venta.\`)}}>Crear y seleccionar cliente</button></div>}{saleClientId&&<small style={{display:'block',marginTop:8}}>Cliente seleccionado: <b>{clients.find(c=>c.id===saleClientId)?.name||'—'}</b></small>}</div>{products.length?<div className="product-grid sale-grid">{products.map(p=><SaleCard key={p.id} product={p} onSell={registerSale}/>)}</div>:<div className="empty-state">Primero agregá productos desde Productos.</div>}<div className="table sales-history"><h3>Historial de ventas</h3>{sales.slice(0,30).map(s=><div className="trow" key={s.id}><span><b>{s.product}</b><small>{s.quantity||1} unidad{(s.quantity||1)===1?'':'es'} · {s.payment||'Efectivo'}{s.clientName?\` · Cliente: \${s.clientName}\`:' · Cliente no registrado'}</small></span><span>{new Date(s.date).toLocaleString('es-AR')}</span><b>{money(s.amount)}</b></div>)}{!sales.length&&<div className="empty-state">Todavía no registraste ventas.</div>}</div></section>}`
source = source.slice(0, salesStart) + salesBlock + source.slice(clientsStart)

// Ventas: permitir escribir cualquier cantidad positiva. No limitar el input al stock;
// registerSale ya valida el stock y muestra una alerta cuando se excede.
const saleCardPattern = /function SaleCard\([\s\S]*$/
const saleCardReplacement = `function SaleCard({product,onSell}:{product:Product;onSell:(p:Product,q:number,payment:string)=>void}){const [quantity,setQuantity]=useState('1');const [payment,setPayment]=useState('Efectivo');const numericQuantity=Number(quantity);const total=product.price*(Number.isFinite(numericQuantity)&&numericQuantity>0?numericQuantity:0);const max=product.stock;const validQuantity=Number.isInteger(numericQuantity)&&numericQuantity>0;const overStock=validQuantity&&numericQuantity>max;return <article className="mini-product sale-card"><div className="mini-art">{product.image?<img src={product.image} alt={product.name}/>:<Icon name="sales" size={36}/>}</div><h3>{product.name}</h3><small>SKU {product.sku||'Sin SKU'} · Stock disponible: <b>{product.stock}</b></small><div className="sale-controls"><label>Cantidad<input type="number" min="1" step="1" value={quantity} onChange={e=>setQuantity(e.target.value)} onBlur={()=>{if(quantity!==''&&!validQuantity)setQuantity('1')}} placeholder="1"/></label><label>Medio de pago<select value={payment} onChange={e=>setPayment(e.target.value)}><option>Efectivo</option><option>Transferencia</option><option>Mercado Pago</option><option>Tarjeta</option><option>Otro</option></select></label></div>{overStock&&<div className="auth-message">⚠️ Stock insuficiente: hay {max} disponible{max===1?'':'s'}.</div>}<div className="sale-total"><span>Total</span><b>{money(total)}</b></div><button className="primary-btn" onClick={()=>{if(!validQuantity){window.alert('⚠️ Ingresá una cantidad entera mayor a 0.');return}if(numericQuantity>max){window.alert('⚠️ Stock insuficiente. Solo hay '+max+' unidades disponibles.');return}onSell(product,numericQuantity,payment)}} disabled={!product.stock||product.active===false}>REGISTRAR VENTA</button></article>}`
if (saleCardPattern.test(source)) source = source.replace(saleCardPattern, saleCardReplacement)

fs.writeFileSync(file, source)
console.log('CASA ALLEGRA: selección de cliente agregada al módulo de Ventas sin formularios anidados y cantidad libre con alerta de stock.')
