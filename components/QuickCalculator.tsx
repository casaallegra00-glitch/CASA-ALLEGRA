...
export default function QuickCalculator(){
 ...
 return <section style={wrap}>
  ...
  <div style={calc}><div style={screen}>{display}</div><div style={memoryRow}>{key('MC',memoryClear)}{key('MR',memoryRecall)}{key('M−',memorySubtract)}{key('M+',memoryAdd)}</div><div style={grid}>{key('AC',clear)}{key('±',toggleSign)}{key('%',percent)}{key('÷',()=>chooseOp('÷'))}{key('7',()=>enter('7'))}{key('8',()=>enter('8'))}{key('9',()=>enter('9'))}{key('×',()=>chooseOp('×'))}{key('4',()=>enter('4'))}{key('5',()=>enter('5'))}{key('6',()=>enter('6'))}{key('−',()=>chooseOp('-'))}{key('1',()=>enter('1'))}{key('2',()=>enter('2'))}{key('3',()=>enter('3'))}{key('+',()=>chooseOp('+'))}{key('0',()=>enter('0'),true)}{key('.',decimal)}{key('=',equals)}</div></div>
  ...
  <div style={hint}><b>Ejemplo:</b> 100 → + → 20 → % → = da <b>120</b>. Cada resultado confirmado se guarda automáticamente en el historial.</div>
 </section>
}
