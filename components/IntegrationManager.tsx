...
export default function IntegrationManager({onNotice}:Props){
 ...
 const refreshConnection=async(key:IntegrationKey)=>{
  try{const res=await fetch(`/api/integrations/${key}`,{cache:'no-store'});const data=await res.json();setConnected(c=>({...c,[key]:Boolean(data.connected)}))}catch{}
 }
 ...
 const loadRemote=async(key:IntegrationKey)=>{
  setLoading(true)
  try{
   if(key==='mercadopago'&&!connected.mercadopago){window.location.href='/api/integrations/mercadopago/connect';return}
 ...
 return <section className='panel large-section'>
  ...
  <div className='panel' style={{marginBottom:18}}>
   <div className='panel-heading'><div><h3>{title}</h3><small>{connected[active]?'Conexión activa':'Conectá la cuenta para habilitar la sincronización.'}</small></div><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><button type='button' className='secondary-btn' onClick={()=>setShowSetup(v=>!v)}>{showSetup?'Cerrar configuración':'Configurar'}</button><button type='button' className='primary-btn' onClick={()=>void loadRemote(active)} disabled={loading}>{active==='mercadopago'&&!connected.mercadopago?'Conectar Mercado Pago':loading?'Actualizando…':'Sincronizar'}</button></div></div>
